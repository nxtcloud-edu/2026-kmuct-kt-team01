...
**현재 Vite 설정에는 `/api` 프록시가 없습니다.** 따라서 Vite 주소에 쿼리만 붙이는 것으로 별도 8000 포트의 백엔드가 연결되지는 않습니다. [nginx.conf](nginx.conf)처럼 정적 프론트와 `/api` 프록시를 같은 origin에서 제공해야 합니다. 배포 환경에서는 `http://<EC2_IP>/?data=api`로 접속합니다.
...
- 이 README는 `main`의 `d469d490892233a8cf20575a3c6e411534131bdb` 코드를 기준으로 작성했습니다.
